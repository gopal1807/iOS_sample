...
//  FunnyCow
//
//  Created by Gopal Krishan on 16/11/21.
//

import SwiftUI

fileprivate func flag(country:String) -> String {
...
fileprivate let countryDictionary = ["AF":"93","AL":"355","DZ":"213","US":"1",
...
struct PhoneNumberRoundedCorner: View {
    @State var countryFlag = "🇺🇸"
    let title: String
    let text: Binding<String>
    let countryCode: Binding<String>
    @State var isPresented = false
    init(title: String, text: Binding<String>, countryCode: Binding<String>) {
        self.title = title
        self.text = text
        let key = countryDictionary.first(where: {$1 == countryCode.wrappedValue})?.key ?? "US"
        self.countryFlag = flag(country: key)
        self.countryCode = countryCode
    }
    var body: some View {
        let textColor = Color(#colorLiteral(red: 0.47882092, green: 0.47882092, blue: 0.47882092, alpha: 1))
        
        HStack {
            Text("\(countryFlag) +\(countryCode.wrappedValue)")
                .padding(5)
        ...
        .roundedCorner(color: Color.gray.opacity(0.3))
        .sheet(isPresented: $isPresented) {
            CountryCodeView(countryCode: countryCode, countryFlag: $countryFlag, y: $isPresented)
        }
    }
}
