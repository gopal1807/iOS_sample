...
//  FunnyCow
//
//  Created by Gopal Krishan on 12/11/20.
//
...
struct SignupVerifyOtpView: View {
    ...
    init(otpReq: NewUserRequest) {
        _viewModel = StateObject(wrappedValue: SignupVerifyOtpViewModel(otpReq))
    }
    
    @StateObject var viewModel: SignupVerifyOtpViewModel
    ...
    var body: some View {
        ZStack {
            VStack {
                ...
                Label(viewModel.otpReq.data.customerData.eMail, systemImage: "envelope.fill")
                ZStack {
                    HStack {
                        ForEach(0 ..< 4) { item in
